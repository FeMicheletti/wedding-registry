	...
	return (
		<section className="space-y-5">
			...
			<div className="inline-flex rounded-xl border border-zinc-200 bg-white p-1 shadow-sm">
				<button type="button" onClick={() => setActiveTab("contributions")} className={`rounded-lg px-4 py-2 text-sm font-medium transition ${activeTab === "contributions" ? "bg-zinc-950 text-white shadow-sm" : "text-zinc-600 hover:bg-zinc-100 hover:text-zinc-950"}`}>
					Presentes
				...
				<button type="button" onClick={() => setActiveTab("letters")} className={`rounded-lg px-4 py-2 text-sm font-medium transition ${activeTab === "letters" ? "bg-zinc-950 text-white shadow-sm" : "text-zinc-600 hover:bg-zinc-100 hover:text-zinc-950"}`}>
					Cartinhas
					<span className="ml-2 rounded-full bg-white/15 px-2 py-0.5 text-xs">
						{letters.length}
					</span>
